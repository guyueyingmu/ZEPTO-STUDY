...
var Zepto = (function () {
	...
	function type(obj) {
		return obj == null ? String(obj) :
			class2type[toString.call(obj)] || "object" // 返回obj的变量类型，以小写字母来表示，
	}
	 ...
	 *  @method isWindow
	 *  @param  {Object}  obj 传入判断的对象
	 *  @return {Boolean}     返回最后判断的结果
	 */
	...
	$.fn = {
		...
		ready: function (callback) {
			...
			if (readyRE.test(document.readyState) && document.body) callback($) // 如果document.body 存在，就直接调用
				...
				callback($)
			}, false) // 没有的话 监听 DOMContentLoaded 事件dom 文档加载完成后再执行。
			return this
		},
		// 获取数组中的指点项
		get: function (idx) {
	...
	['width', 'height'].forEach(function (dimension) {
		...
		// 在fn 原型上绑定即将生成的 width,和  height 两个函数
		$.fn[dimension] = function (value) {
			var offset, el = this[0]
			// 没有传入value值，就表示要获取元素的值，
			//  isWindow   使用innerwidth innerheight
			//  isDocument  使用scrollheight    scrollwidth;
			if (value === undefined) return isWindow(el) ? el['inner' + dimensionProperty] :
				isDocument(el) ? el.documentElement['scroll' + dimensionProperty] :
				(offset = this.offset()) && offset[dimension]
			// 如果参数中传入了value ，则代表使用zepto 中的 css 方法来设置对应的值。
	...
	adjacencyOperators.forEach(function (operator, operatorIndex) {
		...
		$.fn[operator] = function () {
			...
			//  如果传入的参数 的长度小于1 表示没有插入任何值，可以直接返回对应
			if (nodes.length < 1) return this

			return this.each(function (_, target) {
				// after ，before 插入 需要 target。parentNode  父类
				// prepend  append  插入 不需要获取父类
				parent = inside ? target : target.parentNode

				// convert all methods to a "before" operation
				// 把所有的dom 操作都转化成  before 插入方法进行dom操作
				target = operatorIndex == 0 ? target.nextSibling : // after  则 表示target节点之后紧跟的节点之前使用before 方法。
					operatorIndex == 1 ? target.firstChild : // prepend  则 表示target节点子元素的头部插入元素
					operatorIndex == 2 ? target : //  before 直接使用before 来进行操作。
					null //  append   则返回 null

				var parentInDocument = $.contains(document.documentElement, parent)
				// 遍历要插入内容
				nodes.forEach(function (node) {
					// 如果要在不止一个节点上执行 插入node 内容的操作，则首先要将待插入的node 进行
					//  cloneNode 的参数 deep    --> true: 则该节点的所有后代节点也都会被克隆， ---》 false: 则只克隆该节点本身。
					if (copyByClone) node = node.cloneNode(true)
					else if (!parent) return $(node).remove() // 没有任何插入依据的父元素，无法进行
